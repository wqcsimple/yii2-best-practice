...
class BaseApiController extends Controller
{
    ...
    public function beforeAction($action)
    {
        ...
        $this->checkParams(['admin', 'version']);
        $this->client = intval($this->params['admin']);
        $this->version = intval($this->params['version']);

        //verify token, update token time and get user_id
        $this->user_id = ApiService::before($this->params, $this->action->id, $this->action->controller->id);
    ...
    {
        ApiService::checkParams($this->params, $requiredParams);
    }
    ...
    public function log($title, $data, $key = 'smartwork-oa-log')
    {
        $log = [
            'time' => DXUtil::timeFormat(time()),
            'title' => $title,
            'data' => $data
        ];
        $redis = Redis::client();
